...
export default function Raffle({ walletConnected, ystBalance, onNavigate }: Props) {
  ...
  const raffleCreate = () => {
    if (!rfName || !rfPrice || !rfMax || !rfEnd || !rfWallet) {
      setRfStatus('⚠ Please fill in all fields.');
      return;
    }
    if (!walletConnected) {
      setRfStatus('⚠ Connect your wallet first.');
      return;
    }
    ...
    setRfStatus('✓ Raffle launched! Go live instantly after 0.05 SOL fee is confirmed.');
  ...
  return (
    <div className="sec-pad">
      <div className="sec-eyebrow">YAKK — NFT RAFFLE ENGINE</div>
      <div className="sec-title">NFT Raffles 🎟️</div>
      ...
      <div className="raffle-grid" style={{ marginBottom: 28 }}>
        {activeRaffles.length === 0 ? (
          <div style={{ fontFamily: 'Space Mono,monospace', fontSize: 9, color: 'var(--dim)', padding: '28px 0', gridColumn: '1 / -1', textAlign: 'center' }}>
            No active raffles yet. Create one below. 👇
          </div>
        ) : (
          activeRaffles.map(r => (
            <div key={r.id} className="card" style={{ padding: '16px 18px' }}>
              ...
              <button className="btn btn-gold" style={{ fontSize: 11, width: '100%', justifyContent: 'center' }}>
                🎟️ Buy Ticket — {r.ticketPrice} SOL
              </button>
            </div>
          ))
        )}
      ...
      <div className="raffle-create-card">
        <div className="raffle-create-title">🎟️ Create a Raffle</div>
        ...
        <input
          ...
          id="rf-name"
          type="text"
          placeholder="e.g. YAKK Genesis #001 — 1/1 hand-drawn"
          value={rfName}
        ...
        <div style={{ background: 'rgba(247,201,72,0.07)', border: '1px solid rgba(247,201,72,0.2)', borderRadius: 7, padding: '11px 14px', margin: '14px 0', fontSize: 11, color: 'var(--muted)', lineHeight: 1.7 }}>
          <strong style={{ color: 'var(--gold)' }}>Setup fee: 0.05 SOL</strong> → YAKK Studios treasury.<br />
        ...
        >
          🎟️ PAY 0.05 SOL &amp; LAUNCH RAFFLE
        </button>

        {rfStatus && (
          <div className="inv-status" style={{ marginTop: 10, fontSize: 12, color: rfStatus.startsWith('✓') ? 'var(--green)' : 'var(--red)', fontFamily: 'Space Mono,monospace' }}>
            {rfStatus}
          </div>
        )}
      </div>
