...
export default function TgBot({ walletConnected, ystBalance, onNavigate }: Props) {
  ...
  const strategyLabels: Record<string, string> = {
    ...
    sniper: 'New Token Sniper',
    arb: 'DEX Arbitrage',
    custom: 'Custom (AI-built)',
  };

  const buildBot = () => {
    if (!walletConnected) return;
    const name = botName || 'YAKKBot';
    const strat = strategyLabels[strategy] || strategy;
    const toksStr = tokens || '$YST, SOL';
    const maxStr = maxTrade ? `$${maxTrade}` : 'not set';
  ...
  return (
    <div className="sec-pad">
      <div className="sec-eyebrow">AI-POWERED</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div className="card-sm">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 9, color: 'var(--dim)', marginBottom: 5, letterSpacing: '0.08em' }}>BOT NAME</div>
              <input type="text" placeholder="e.g. YAKKBot Alpha" value={botName} onChange={e => setBotName(e.target.value)} style={inputStyle} />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <div>
                <div style={{ fontSize: 9, color: 'var(--dim)', marginBottom: 5 }}>MAX TRADE SIZE ($)</div>
                <input type="number" placeholder="e.g. 500" value={maxTrade} onChange={e => setMaxTrade(e.target.value)} style={{ ...inputStyle, fontSize: 12 }} />
              </div>
              <div>
                <div style={{ fontSize: 9, color: 'var(--dim)', marginBottom: 5 }}>CONFIRM ABOVE ($)</div>
                <input type="number" placeholder="e.g. 200" value={confirmAbove} onChange={e => setConfirmAbove(e.target.value)} style={{ ...inputStyle, fontSize: 12 }} />
              </div>
            </div>
            ...
            <div>
              <div style={{ fontSize: 9, color: 'var(--dim)', marginBottom: 5 }}>DAILY LOSS LIMIT ($)</div>
              <input type="number" placeholder="Bot pauses if this is hit" value={lossLimit} onChange={e => setLossLimit(e.target.value)} style={{ ...inputStyle, fontSize: 12 }} />
            </div>

            <div>
              <div style={{ fontSize: 9, color: 'var(--dim)', marginBottom: 5 }}>TOKENS TO TRADE</div>
              <input type="text" placeholder="$YST, $SPT, SOL, or token addresses" value={tokens} onChange={e => setTokens(e.target.value)} style={inputStyle} />
            </div>
        ...
        {/* Right: Safety + preview + Telegram mock */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* Bot preview (shows when built) */}
          {preview && (
            <div className="card-sm">
              <div style={{ fontFamily: 'Space Mono,monospace', fontSize: 9, color: 'var(--dim)', letterSpacing: '0.12em', marginBottom: 12 }}>BOT PREVIEW</div>
              <div style={{ fontSize: 11, color: 'var(--text)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{preview}</div>
            </div>
          )}
