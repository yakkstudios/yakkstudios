      ...
      {hasAccess && (
        <div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
            <input
              className="field-inp"
              placeholder="Enter wallet address or @twitter handle..."
              value={addr}
          ...
          {result?.found && addr && (
            <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, padding: 20 }}>
              ...
              <div className="grid2" style={{ marginBottom: 16 }}>
                {[
                  { l: 'SOL BALANCE', v: '12.4 SOL', c: 'var(--green)' },
                  { l: 'TOKEN COUNT', v: '34 tokens', c: 'var(--text)' },
                  { l: 'TOTAL VALUE', v: '$2,847', c: 'var(--gold)' },
                  { l: 'RISK SCORE', v: '3/10 LOW', c: 'var(--green)' },
                ].map(s => (
              ...
              {[['$SOL','Solana','12.4','$2,108'],['$YST','YAKK Studios','2.1M','$378'],['$BONK','Bonk','420K','$9']].map(([sym,name,amt,val]) => (
                <div key={sym} className="lb-row" style={{ marginBottom: 6 }}>
                  <span className="td-tok" style={{ minWidth: 60 }}>{sym}</span>
                  <span style={{ flex: 1, color: 'var(--muted)', fontSize: 11 }}>{name}</span>
                  <span style={{ fontFamily: 'Space Mono,monospace', fontSize: 10, color: 'var(--text)' }}>{amt}</span>
                  <span style={{ fontFamily: 'Space Mono,monospace', fontSize: 10, color: 'var(--green)', minWidth: 60, textAlign: 'right' }}>{val}</span>
                </div>
              ))}
          ...
          <div style={{ marginTop: 20 }}>
            ...
            {[
              { label: 'Ansem (Alpha Leaks)', addr: '7oKJ...MMSk', risk: 'MEDIUM', badge: 'b-gold' },
              { label: 'ZachXBT (Fraud Hunter)', addr: 'Fh V...HLuM', risk: 'TRUSTED', badge: 'b-green' },
              { label: 'Tier 1 KOL [CABAL]', addr: '6zNu...Ae9q', risk: 'HIGH RISK', badge: 'b-red' },
            ].map(w => (
              <div key={w.addr} className="lb-row" style={{ marginBottom: 6, cursor: 'pointer' }} onClick={() => setAddr(w.addr)}>
                <span style={{ flex: 1, fontSize: 12 }}>{w.label}</span>
                <span style={{ fontFamily: 'Space Mono,monospace', fontSize: 10, color: 'var(--dim)', marginRight: 10 }}>{w.addr}</span>
                <span className={`badge ${w.badge}`}>{w.risk}</span>
              </div>
            ))}
          </div>
        </div>
