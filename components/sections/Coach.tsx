...
const MODE_LABELS: Record<Mode, string> = { discipline: 'DISCIPLINE MODE', raid: 'RAID STRATEGIST', risk: 'RISK OFFICER', lore: 'YAKK LORE' };
...
export default function Coach({ walletConnected, ystBalance, onNavigate }: Props) {
  ...
  const [msgs, setMsgs] = useState([{ role: 'ai', text: WELCOME_MSG['discipline'] }]);
  const [input, setInput] = useState('');
  const [loading, setLoading] = useState(false);
  const handleSetMode = (newMode: Mode) => { setMode(newMode); setMsgs([{ role: 'ai', text: WELCOME_MSG[newMode] }]); };
  const sendMsg = (text?: string) => {
    const q = (text || input).trim(); if (!q) return;
    setInput(''); setMsgs(m => [...m, { role: 'user', text: q }]); setLoading(true);
    setTimeout(() => { const reply = AI_RESPONSES[q] || `${MODE_LABELS[mode]} responding: "${q}" — The yakk sees all. Position size wisely, stay in the den, and remember: the mountain rewards patience.`; setMsgs(m => [...m, { role: 'ai', text: reply }]); setLoading(false); }, 900);
  };
  return (
    <div className="sec-pad">
      ...
      <div style={{ maxWidth: 640 }}>
        <div style={{ display: 'flex', gap: 7, marginBottom: 16, flexWrap: 'wrap' }}>
          {(['discipline', 'raid', 'risk', 'lore'] as Mode[]).map(m => (<div key={m} className={`mode-pill${mode === m ? ' active' : ''}`} onClick={() => handleSetMode(m)} style={{ cursor: 'pointer' }}>{MODE_LABELS[m]}</div>))}
        </div>
        <div className="chat-msgs" id="chat-msgs">
          {msgs.map((msg, i) => (<div key={i} className={msg.role === 'user' ? 'msg-user' : 'msg-ai'}>{msg.role === 'ai' && <div className="ai-lbl">YAKKAI — {MODE_LABELS[mode]}</div>}{msg.text}</div>))}
          {loading && (<div className="msg-ai"><div className="ai-lbl">YAKKAI — {MODE_LABELS[mode]}</div><span style={{ opacity: 0.6, fontFamily: 'Space Mono,monospace', fontSize: 11 }}>thinking...</span></div>)}
        </div>
        ...
        <div style={{ display: 'flex', gap: 7 }}>
          <input className="chat-inp" type="text" placeholder="Ask YAKKAI anything..." value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMsg()} />
