...
export default function Predictions({ walletConnected, ystBalance, onNavigate }: Props) {
  ...
  const handleChat = () => { if (!chatInput.trim()) return; setChatMsgs(p=>[...p,{role:'user',text:chatInput}]); const reply=`YAKKAI analysis: current sentiment is moderately bullish. On-chain signals show accumulation. Community conviction: HIGH. Always DYOR.`; setChatMsgs(p=>[...p,{role:'ai',text:reply}]); setChatInput(''); };
  const handleGetPick = () => setAiPick('YAKKAI TOP PICK: "Will Bitcoin hit $150K before July 2025?" — YES at 62%. YAKKAI signal: STRONG BUY on YES. Whale accumulation, ETF inflow momentum sustained. Confidence: 87%.');
  ...
  return (
    <div className="sec-pad">
      <div className="sec-eyebrow">10 — YAKKAI PREDICTION ENGINE</div><div className="sec-title">PREDICTION <span style={{color:'var(--gold)'}}>MARKETS</span></div><div className="sec-bar" />
      <div className="card" style={{marginBottom:24,borderColor:'rgba(247,201,72,0.25)'}}>
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:14}}><span style={{fontSize:22}}>🔮</span><div><div style={{fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:14,color:'var(--gold)'}}>YAKKAI TOP PICK</div></div><button className="btn btn-gold" style={{marginLeft:'auto',fontSize:9,padding:'6px 13px'}} onClick={handleGetPick}>🤖 GET AI PICK</button></div>
        <div style={{minHeight:80,background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:7,padding:13,fontSize:12,color:'var(--muted)',lineHeight:1.7}}>{aiPick||<span style={{fontFamily:'Space Mono,monospace',fontSize:9,color:'var(--dim)'}}>Click "Get AI Pick" to see YAKKAI's top signal...</span>}</div>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:20,alignItems:'start'}}>
        ...
        <div>
          ...
          <div className="card"><div style={{fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:12,marginBottom:12}}>💬 ASK YAKKAI</div><div style={{height:160,overflowY:'auto',background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:8,padding:14,display:'flex',flexDirection:'column',gap:10,marginBottom:10}}>{chatMsgs.map((msg,i)=>(msg.role==='ai'?<div key={i} style={{alignSelf:'flex-start',background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'8px 8px 8px 2px',padding:'8px 13px',maxWidth:'80%',fontSize:12,lineHeight:1.6}}>{msg.text} 🔮</div>:<div key={i} style={{alignSelf:'flex-end',background:'rgba(224,96,126,0.1)',border:'1px solid rgba(224,96,126,0.2)',borderRadius:'8px 8px 2px 8px',padding:'8px 13px',maxWidth:'80%',fontSize:12}}>{msg.text}</div>))}</div><div style={{display:'flex',gap:7}}><input placeholder="Ask about any market..." value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')handleChat();}} style={{flex:1,background:'var(--bg4)',border:'1px solid var(--border)',borderRadius:5,padding:'8px 12px',color:'var(--text)',fontSize:12,outline:'none'}} /><button className="btn btn-gold" style={{fontSize:10,padding:'6px 11px'}} onClick={handleChat}>ASK</button></div></div>
        </div>
      </div>
    </div>
  );
